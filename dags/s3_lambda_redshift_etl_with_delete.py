"""
S3 → Lambda → Redshift ETL DAG (with table delete)

Generates dummy data across 5 parallel task instances, uploads CSVs to S3,
invokes a Lambda to consolidate them, deletes existing Redshift data,
then loads into Redshift.
...
with DAG(
    dag_id="s3_lambda_redshift_etl_with_delete",
    ...
    # Delete all rows from the Redshift table before loading fresh data
    delete_redshift_table = RedshiftDataOperator(
        task_id="delete_redshift_table",
        database=REDSHIFT_DATABASE,
        workgroup_name=REDSHIFT_WORKGROUP,
        sql=f"DELETE FROM {REDSHIFT_TABLE};",
        aws_conn_id="aws_default",
        region_name="us-east-1",
